...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph_type='GAT', layer=1, path_list=None, data_path=None):
        ...
        if graph_type == 'GAT' or graph_type == 'GCN':
            self.label_num = config.num_labels - 3
        elif graph_type == 'graphormer':
        ...
        topN_keywords_dict = torch.load(os.path.join(data_path, 'topN_keywords_dict.pt'))
        keywords = []
        for i in range(len(topN_keywords_dict)):
            token_kv = topN_keywords_dict[i]
            tokens = []
            for j in range(len(token_kv)):
                tokens.append(token_kv[j][0])
            keywords.append(tokens)
        self.label_keywords = keywords


        if self.graph_type == 'graphormer':
            self.inverse_label_list = {}

            def get_root(path_list_kid2parent, n):
                ret = []
                # while path_list[n] != n:
                #     ret.append(n)
                #     n = path_list[n]
                while path_list_kid2parent[n] != -1:
                    ret.append(n)
                    n = path_list_kid2parent[n]
                ret.append(-1)
                return ret

            # add the path, where link kid to parent node
            path_list_kid2parent = [-1 for i in range(self.label_num)]
            # print(path_list)
            for item in path_list:
                id_kid = item[1]
                id_parent = item[0]
                path_list_kid2parent[id_kid] = id_parent


            for i in range(self.label_num):
                self.inverse_label_list.update({i: get_root(path_list_kid2parent, i)})
            ...
            label_keywords_ids_temp = self.tokenizer(self.label_keywords, is_split_into_words=True)['input_ids']
            self.label_keywords_ids = []
            for i in range(len(label_keywords_ids_temp)):
                id_list = []
                for j in range(len(label_keywords_ids_temp[i])):
                    if label_keywords_ids_temp[i][j] not in [101, 102]:
                        id_list.append(label_keywords_ids_temp[i][j])
                if len(id_list) == 0:
                    id_list.append(0)
                self.label_keywords_ids.append(id_list)

            self.label_co_ratio = torch.tensor(torch.load(os.path.join(data_path,'label_co_ratio.pt'))).view(1, -1)
            self.label_co_ratio = nn.Parameter(self.label_co_ratio, requires_grad=False)
            self.label_co_ratio_embedding = nn.Embedding(10+1, 1, 0)
    ...
    def forward(self, label_emb, embeddings):
        ...
        if self.graph_type == 'graphormer':
            ...
            for i in range(len(self.label_keywords_ids)):
                keyword_emb = embeddings(torch.tensor(self.label_keywords_ids[i], dtype=torch.long, device=device)).mean(dim=0)
                label_keywords_emb.append(keyword_emb)

            label_keywords_emb = torch.stack(label_keywords_emb, dim=0)
            label_id_emb = self.id_embedding(self.label_id[:, None]).view(-1,self.config.hidden_size)
            # label_emb = label_emb + label_id_emb + label_ratio_emb
            label_emb = label_emb + label_id_emb

            distance_att = self.distance_embedding(self.distance_mat)
            edge_att = self.edge_embedding(self.edge_mat).sum(dim=1) / (self.distance_mat.view(-1, 1) + 1e-8)
            label_co_ratio_att = self.label_co_ratio_embedding(self.label_co_ratio)
            extra_attn = distance_att+edge_att+label_co_ratio_att
