...
import os
from .loss import multilabel_categorical_crossentropy,multilabel_categorical_BCE_weighted
...
class GraphEmbedding(nn.Module):
    def __init__(self, config, embedding, new_embedding, graph_type='GAT', layer=1, path_list=None, data_path=None, depth2label=None):
        ...
        new_embedding = torch.cat(
            [torch.zeros(1, new_embedding.size(-1), device=new_embedding.device, dtype=new_embedding.dtype),
             new_embedding], dim=0)  # 1 zero + 166 label + 8 depth + 1 profix =176
    ...
    def weight(self):
        def foo():
            ...
            if self.graph_type == 'GAT':
                ...
                edge_features = edge_features[:self.num_class, :]
                edge_features = self.graph(edge_features, self.original_embedding)
                prompt_features = []
                for i in range(len(self.depth2label)):
                    labels = self.depth2label[i]
                    label_features = []
                    for l in labels:
                        label_features.append(edge_features[l])
                    prompt_f = torch.stack(label_features).mean(dim=0) + self.new_embedding.weight[self.num_class+i]
                    prompt_features.append(prompt_f)
                prompt_features = torch.stack(prompt_features)
                edge_features = torch.cat(
                    [edge_features,prompt_features, self.new_embedding.weight[-1:, :]], dim=0)  # 166 class + 1 profix
...
class Prompt(BertPreTrainedModel):
    ...
    def __init__(self, config, graph_type='GAT', layer=1, path_list=None, data_path=None, depth2label=None, use_weighted_loss=True, **kwargs):
    ...
    #   the weight for the loss
        ...
        self.class_weights = [1000 for _ in range(config.num_labels)]
    ...
    def init_embedding(self):
        ...
        label_emb = []
        input_embeds = self.get_input_embeddings() # 30522 768
        ...
        if self.graph_type == 'GAT' or self.graph_type == 'GCN':
            # label prompt mask
            label_emb = torch.cat(
                [torch.stack(label_emb), prompt_embedding.weight[1:, :], prefix], dim=0)  # 175 = 166+8+1
        elif self.graph_type == 'graphormer':
            label_emb = torch.cat(
                [torch.stack(label_emb), prompt_embedding.weight[1:, :], prefix], dim=0)  # 175 = 166+8+1
        embedding = GraphEmbedding(self.config, input_embeds, label_emb, self.graph_type,
                                   path_list=self.path_list, layer=self.layer, data_path=self.data_path, depth2label=self.depth2label)

        self.set_input_embeddings(embedding)
        output_embeddings = OutputEmbedding(self.get_output_embeddings().bias)  # this bias is useless,will not update
        ...
        # print(output_embeddings.bias.data)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.use_weighted_loss:
                ...
                labels_temp = labels.masked_select(label_mask).view(-1, multiclass_logits.size(-1))
                labels_temp = labels_temp.cpu().numpy().tolist()
                for l in labels_temp:
                    weight_add = [3 if item > 0 else 0 for item in l]
                    # self.weight_class = weight_add + self.weight_class
                    for i in range(len(self.class_weights)):
                        self.class_weights[i] += weight_add[i]
                weight_temp = [1 if item > 0 else 0 for item in self.class_weights]
                # compute the loss
                multiclass_loss = multilabel_categorical_BCE_weighted(labels.view(-1, self.num_labels),
                                                                               multiclass_logits, weight_temp)
                # update the class_weight use the negative
                for l in labels_temp:
                    weight_reduce = [1 if item == 0 else 0 for item in l]
                    for i in range(len(self.class_weights)):
                        if self.class_weights[i] > 0:
                            self.class_weights[i] -= weight_reduce[i]
            else:
                multiclass_loss = multilabel_categorical_crossentropy(labels.view(-1, self.num_labels),
                                                                      multiclass_logits)
